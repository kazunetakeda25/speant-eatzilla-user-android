...
public class ItemListHistory implements Parcelable {
    ...
    private List<FoodSize> food_size;

    private List<AddOns> add_ons;

    protected ItemListHistory(Parcel in) {
        ...
        food_size = in.createTypedArrayList(FoodSize.CREATOR);
        add_ons = in.createTypedArrayList(AddOns.CREATOR);
    }

    public static final Creator<ItemListHistory> CREATOR = new Creator<ItemListHistory>() {
        @Override
        public ItemListHistory createFromParcel(Parcel in) {
            return new ItemListHistory(in);
        }

        @Override
        public ItemListHistory[] newArray(int size) {
            return new ItemListHistory[size];
        }
    };
    ...
    public List<FoodSize> getFood_size() {
        return food_size;
    }

    public void setFood_size(List<FoodSize> food_size) {
